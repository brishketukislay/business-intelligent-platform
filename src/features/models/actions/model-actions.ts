...
function formDataToBusinessModel(
  ...
  return {
    ...
    itemLabelSingular: String(
      formData.get("itemLabelSingular") ?? "Item",
    ).trim(),

    itemLabelPlural: String(
      formData.get("itemLabelPlural") ?? "Items",
    ).trim(),
  };
}
...
export async function createBusinessModelAction(
  ...
  const data = formDataToBusinessModel(formData);

  const result =
    businessModelSchema.safeParse(data);
...
export async function updateBusinessModelAction(
  ...
  try {
    ...
    revalidatePath(`/models/${modelId}/edit`);
